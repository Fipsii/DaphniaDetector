...
### This is the NON-DOCKER VERSION
### You need to change the paths or location especially in the YAML data
### But conserve the folder structure

import time
import os
import subprocess
import pandas as pd
from Convert_to_JPG import ConvertToJPEG, EmptyCheck
...
from Pytorch_Deploy import Classify_Species
import warnings
import sys
...
try:
  ...
  # The structur of the files from github is
  # Detector/
  # ├─ Models/
  # │  ├─ resnet18_model_final.pth
  # │  ├─ ...
  # ├─ Code/
  ...
  # ├─ Setup/
  #
  # And we want to save Detector/Code
  
  current_directory = os.path.dirname(os.path.abspath(__file__))
  
  # And we want to save Detector/
  model_directory = os.path.dirname(os.path.dirname(__file__))
  ...
  ### Paths 
  Original_Images = os.path.join(model_directory,settings["Original_images"]) 
  Analysis_data = os.path.join(model_directory,settings["Analysis_data"])
  Classifier_loc = os.path.join(model_directory,settings["Classifier_loc"])
  Cache_Org_Img = Original_Images

  ### Check if user provided data if empty or folder nonexistent exit
  EmptyCheck(Original_Images)
  ...
  if settings["status_flag"] == 0:
    ...
    predict_command = f'PYTHONWARNINGS="{IgnoredWarnings}" conda run --no-capture-output -n ginjinnGPU ginjinn predict {model_directory}/Models/Loose_box_final_2204 -i {Original_Images} -o {Analysis_data} {mode}'
  ...
  ScaleDataframe.to_csv(Analysis_data + "/scale.csv", index = False)      
  ...
  if settings["crop"] == True:
    ...
    try:
      if settings["Classify"] == True and "Daphnid" in settings["organs"]:
        Species = Classify_Species(Original_Images, Classifier_loc)
        print(Species)
      else: 
        print("No Species Classification performed")
  ...
  if settings['Body_width_eval'] == True:
    ...
    IgnoredWarnings = "ignore:This overload of nonzero is deprecated"
    predict_command = f'PYTHONWARNINGS="{IgnoredWarnings}" conda run --no-capture-output -n ginjinnGPU ginjinn predict {model_directory}/Models/daphnid_instances_0.1 -i {Original_Images} -o {Analysis_data + "/segmentation"} {mode}'
    subprocess.run(predict_command, check=True, shell=True)
