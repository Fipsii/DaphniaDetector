    ...
    def predict(
        ...
        with NamedTemporaryFile() as tmpfile:
            ...
            for i_img, img_name in enumerate(img_names):
                ...
                # We want to keep only the likeliest box
                    
                # Find the unique class labels
                unique_labels = np.unique(classes)
            
                # Create empty lists to store the selected boxes, scores, and labels
                selected_boxes = []
                selected_scores = []
                selected_labels = []
                ...
                for label in unique_labels:
                    # Get the indices of boxes belonging to the current label
                    label_indices = np.where(classes == label)[0]
            
                    # Get the scores and boxes for the current label
                    label_scores = scores[label_indices]
                    label_boxes = boxes[label_indices]
            
                    # Find the index of the highest score for the current label
                    max_score_index = np.argmax(label_scores)
            
                    # Select the box with the highest score for the current label
                    selected_box = label_boxes[max_score_index]
                    selected_score = label_scores[max_score_index]
            
                    # Add the selected box, score, and label to the corresponding lists
                    selected_boxes.append(selected_box)
                    selected_scores.append(selected_score)
                    selected_labels.append(label)
            
                # Convert the lists of selected boxes, scores, and labels to numpy arrays
                boxes = np.array(selected_boxes)
                scores = np.array(selected_scores)
                classes = np.array(selected_labels)
...
class SimpleGinjinnPredictor:
    ...
    def predict(
        ...
        #### Start of edit for DaphniaDetector
        # We want to keep only the likeliest box NMS
        ...
        scores = np.array(selected_scores)
        classes = np.array(selected_labels)
        ### End of Edit. Author Philipp Kropf
        ...
        torch.cuda.empty_cache()
        print("We do predict:", classes)
        return classes, boxes, masks, keep
