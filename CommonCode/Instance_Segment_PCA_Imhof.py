...
def Create_Mask(anno_save_loc, parent_dir):
  ...
  ### Create a mask based on the annotations and the the original images
  ### Input: anno_save_loc: Location ofthe annotations
  ###        parent_dir: Location of the image directory
  import json
  import cv2 as cv
  import numpy as np
  import pandas as pd
...
def Create_Visualization_Data(images_sort, X_start, X_end, ListofMidpoints, angles, Rotated_masks, Mask):
    ...
    if angles != 0: ## If images are turned and we provide the input
      ...
      ### Where do we get the y data from do we input it
      EntryY = ListofMidpoints[x][1]
      ExitY = ListofMidpoints[x][1]
      EntryX = X_start[x]
      ExitX = X_end[x]
