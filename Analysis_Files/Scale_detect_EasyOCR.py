...
def Sortlist(String_of_numbers):
  ...
  for y in merged_list:
    ...
    try:
      Float_list = float(''.join(converted_list))
    except:
      Float_list = 0.0
      
    numbers_only.append(Float_list)

  return numbers_only

def makeDfwithfactors(list_of_names, Scale_Mode,List_of_scale_numbers=[], list_of_lengths=[], ConvFactor=0.002139):
  ...
  Scale_df = pd.DataFrame(list_of_names, columns =['Name'])
  print(len(LengthOpt),len(Scale_df), len(UnitOpt))
  Scale_df["metric_length"] = UnitOpt
  Scale_df["scale[px]"] = LengthOpt
  print(LengthOpt,UnitOpt)
