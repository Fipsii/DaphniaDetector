...
def build_model(num_classes):
  ...
  model = tf.keras.applications.vgg16.VGG16(include_top=False, input_tensor=x, input_shape = (600,600,3), weights="imagenet")
...
for train, test in kfold.split(inputs, targets):
  ...
  epochs = 7  # Unfreeze some layers to allow fint tuning
...
with open("Results_Kfold_VGG16.txt", "w") as f:
    for s in new_list:
        f.write(str(s) +"\n")
