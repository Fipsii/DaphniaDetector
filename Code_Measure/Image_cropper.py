...
def CropImages(Original_Images, Dataframe, Crop_mode, Save_folder, Image_names):
  ...
  print(list(set(sorted_df['image_id'].to_list()) - set(Image_names)))
  print(list(set(Image_names) - set(sorted_df['image_id'].to_list())))
  for x in Crop_mode:
    temp_crops = []
    ...
    for y in range(len(Original_Images)):
      try:
        ...
        print(f"No body box detected for {Image_names[y]}")
        temp_crops.append(0)
  return temp_crops
